...
if (!sourceUrl) {
  console.error(
    "❌ Error: Silakan masukkan URL database lama sebagai argumen.",
  );
  console.error(
    'Penggunaan: node migrate_db.js "postgresql://user:pass@host:port/db"',
  );
  process.exit(1);
}
...
const tablesToMigrate = [
  ...
  { name: "tryout_package_completions", conflict: "id" },
...
async function runMigration() {
  ...
  try {
    ...
    console.log(
      "\n🧹 Mengosongkan data dummy di database baru sebelum memindahkan data asli...",
    );
    // Menghapus data seeder/dummy agar tidak terjadi duplikasi unique constraint atau ketidaksesuaian UUID
    const tablesToTruncate = tablesToMigrate
      .map((t) => `"${t.name}"`)
      .join(", ");
    await destClient.query(`TRUNCATE TABLE ${tablesToTruncate} CASCADE`);
    console.log("✅ Database baru berhasil dikosongkan.");
    ...
    for (const table of tablesToMigrate) {
      ...
      if (!tableCheck.rows[0].exists) {
        console.log(
          `⏭️  Tabel "${table.name}" tidak ada di database lama. Dilewati.`,
        );
        continue;
      }

      // Ambil kolom-kolom yang ada di database tujuan
      const columnsRes = await destClient.query(
        `
        SELECT column_name
      ...
      const destColumns = columnsRes.rows.map((r) => r.column_name);
      ...
      if (rows.length === 0) {
        console.log(
          `✅ Tabel "${table.name}" kosong di database lama. Tidak ada data untuk dipindahkan.`,
        );
        continue;
      }

      console.log(
        `📦 Ditemukan ${rows.length} data di tabel "${table.name}". Memindahkan...`,
      );
      ...
      for (const row of rows) {
        // Mapping kolom khusus jika struktur berubah
        if (table.name === "users") {
          if (row.full_name !== undefined && row.name === undefined) {
            row.name = row.full_name;
          }
        }

        // Filter kolom agar hanya menyertakan yang ada di database tujuan
        const columns = [];
        const values = [];
        ...
        try {
          ...
          console.error(
            `❌ Gagal memindahkan baris di tabel "${table.name}":`,
            err.message,
          );
        }
      }
      ...
      console.log(
        `✅ Selesai untuk tabel "${table.name}": ${insertedCount} berhasil dipindahkan, ${skippedCount} dilewati (sudah ada).`,
      );
    }
