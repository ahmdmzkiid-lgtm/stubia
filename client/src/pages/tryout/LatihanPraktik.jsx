    ...
    return (
      <div className="min-h-screen bg-[#faf8ff] flex items-center justify-center p-6">
        <div className="max-w-xl w-full bg-white rounded-3xl p-8 border border-[#c2c6d8]/30 shadow-lg text-center">
          ...
          <p className="text-sm text-[#424656] mb-6">Follow akun Stubia dan tag 3 teman di komentar postingan. Setelah disetujui admin, kamu bisa mengakses latihan soal gratis (latihan premium tetap membutuhkan upgrade).</p>
      ...
      <div className="min-h-screen bg-[#faf8ff] flex items-center justify-center p-6">
        <div className="max-w-xl w-full bg-white rounded-3xl p-8 border border-[#c2c6d8]/30 shadow-lg text-center">
          ...
          <div className="flex gap-3 justify-center">
            ...
            <button onClick={() => navigate(`/latihan/${subjectId || ''}`)} className="px-5 py-3 rounded-xl border border-[#e5e7eb] text-[#424656] font-semibold">Kembali</button>
          </div>
        </div>
      </div>
    );
      ...
      <main className="flex-grow pb-20">
        ...
        <div className="flex flex-col gap-3 mb-6">
          {currentQuestion?.question_type === 'complex_mc_tf' ? (
            /* Complex MC True/False - Pernyataan Benar/Salah */
            <div className="bg-white rounded-2xl p-5 border border-[#e0e2f0] shadow-sm">
              ...
              <div className="space-y-3">
                {(currentQuestion?.choices || []).map((choice) => {
                  ...
                  try {
                    const parsed = answers[currentIndex] ? (typeof answers[currentIndex] === 'string' ? JSON.parse(answers[currentIndex]) : answers[currentIndex]) : {};
                  ...
                  return (
                    <div key={choice.id} className="flex flex-col sm:flex-row sm:items-center gap-3 p-4 rounded-xl border border-[#e0e2f0] bg-[#faf8ff]">
                      ...
                      <div className="flex gap-2 self-end sm:self-center flex-shrink-0">
                        <button
                          onClick={() => {
                            const current = answers[currentIndex] ? (typeof answers[currentIndex] === 'string' ? JSON.parse(answers[currentIndex]) : answers[currentIndex]) : {};
                            const updated = { ...current, [choice.label]: true };
                            handleOptionSelect(JSON.stringify(updated));
                          }}
